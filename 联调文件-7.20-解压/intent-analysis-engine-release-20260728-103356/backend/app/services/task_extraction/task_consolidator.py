...
class TaskConsolidator:
    ...
    BUSINESS_JUDGMENT_PATTERN = re.compile(r"经营判断|经营分析|经营复盘|渠道|线索|转化|质量|健康度|投入产出|变差")
    ...
    def _cluster_key(self, candidate: TaskCandidate) -> tuple[str, ...] | None:
        ...
        if complaint_context:
            return None
        if candidate.action == "analyze" and self.BUSINESS_JUDGMENT_PATTERN.search(text):
            return ("business_judgment_analysis",)
    ...
    def _canonical_values(
        ...
        if key == ("business_judgment_analysis",):
            return "analyze", "经营", "分析经营判断"
