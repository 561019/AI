...
class TaskSegmenter:
    ...
    ACTION_PATTERN = re.compile(
        r"查询|获取|调取|拉取|查看|列出|拿出来|拿出|取出来|取出|读取|解析|提取|抽取|筛选|过滤|圈出来|圈出|排序|排名|排行|排列|倒序|升序|降序|从高到低|从低到高|风险高低|整理|归集|汇总|统计|计算|核算|测算|分析|判断|了解|比较|对比|同比|环比|生成|形成|制作|撰写|写|预测|预估|检查|审核|转换|导出|同步|推送|提交|提醒|监控|发起|启动|办理"
    )
    ...
    def _action_is_predicate(self, text: str) -> bool:
        ...
        if not prefix:
            return True
        if re.match(
            r"^(?:按|按照|根据|基于|结合|使用|用|从|通过|把|将|先|首先|随后|接着|然后|再|最后|同时|此外|另外|之后|在).{0,40}$",
            prefix,
        ):
            return True
