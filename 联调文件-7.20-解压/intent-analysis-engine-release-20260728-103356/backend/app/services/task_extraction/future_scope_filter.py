...
@dataclass(frozen=True)
class CurrentScopeFilterResult:
    original_text: str
    filtered_text: str
    removed_clauses: tuple[str, ...]
    current_scope_empty: bool
    has_explicit_exclusion: bool
...
class FutureScopeFilter:
    ...
    FUTURE_PLANNING_PATTERN = re.compile(r"希望支持|支持|规划|计划|考虑|可能考虑|会做|想做|希望")
    EXCLUSION_MARKERS = (
        "目前不包含",
        "目前不包括",
        "本次不考虑",
    ...
    GENERIC_TARGET_PATTERN = re.compile(
        r"^(?:相关|对应|上述|以上|这些|这类|此类|这部分|该部分|此部分|这块|该块)?(?:功能|事项|工作|任务|能力|内容|部分)?$"
        r"|(?:相关|对应|上述|以上|这些|这类|此类|这部分|该部分|此部分|这块|该块).{0,8}(?:功能|事项|工作|任务|能力|内容|部分)"
    )
    ...
    def filter_current_scope(self, text: str) -> CurrentScopeFilterResult:
        kept: list[str] = []
        removed: list[str] = []
        has_explicit_exclusion = bool(self.find_exclusions(text))
        for clause, start, end in self.iter_clauses(text):
            remove_direct = self.is_direct_exclusion_clause(clause)
            remove_planning = self.is_future_planning_clause(clause)
            remove_cancelled = self.should_remove_by_later_exclusion(
                candidate_text=clause,
                original_text=text,
                candidate_start=start,
            )
            remove_future = self.should_remove_candidate_text(
                candidate_text=clause,
                original_text=text,
                candidate_start=start,
                candidate_end=end,
            )
            if remove_direct or remove_planning or remove_cancelled or remove_future:
        ...
        current_task_like = [
            clause
            for clause in kept
            if self.is_task_like(clause) and not self.is_direct_exclusion_clause(clause)
        ]
        return CurrentScopeFilterResult(
            original_text=text,
            filtered_text=filtered,
            removed_clauses=tuple(removed),
            current_scope_empty=has_explicit_exclusion and not current_task_like,
            has_explicit_exclusion=has_explicit_exclusion,
        )

    def remove_excluded_current_scope(self, text: str) -> str:
        return self.filter_current_scope(text).filtered_text

    def text_is_fully_excluded(self, text: str) -> bool:
        return self.filter_current_scope(text).current_scope_empty

    def is_direct_exclusion_clause(self, text: str) -> bool:
        stripped = text.strip(" ，,。；;！？!?")
        if not stripped:
            return False
        for exclusion in self.find_exclusions(stripped):
            target = self.clean_target(exclusion.target_text)
            if not target:
                return True
            if self.is_broad_target(target) or self.is_task_like(target):
                return True
        return False

    def is_future_planning_clause(self, text: str) -> bool:
        stripped = text.strip(" ，,。；;！？!?")
        if not stripped or not self.is_future_scoped(stripped):
            return False
        return bool(self.FUTURE_PLANNING_PATTERN.search(stripped))

    def should_remove_by_later_exclusion(
        self,
        *,
        candidate_text: str,
        original_text: str,
        ...
        if not self.is_task_like(candidate_text):
            return False
