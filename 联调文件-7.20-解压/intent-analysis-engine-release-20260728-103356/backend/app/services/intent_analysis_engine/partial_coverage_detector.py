...
class PartialCoverageDetector:
    ...
    _SPLIT_RE = re.compile(
        r"(?:\s*[，,。；;！？!?]\s*|\s*(?:然后|接着|随后|最后|并且|并)\s*|\s+\b(?:then|next|finally|and)\b\s+)",
        flags=re.IGNORECASE,
    )
    ...
    def _append_segment(
        ...
        stripped = raw.strip()
        stripped = re.sub(r"^(?:and|then|next|finally)\s+", "", stripped, flags=re.IGNORECASE)
        if not stripped:
            return
        if self._is_embedded_communication_instruction(stripped):
            return
    ...
    def _is_embedded_communication_instruction(self, text: str) -> bool:
        return bool(
            re.fullmatch(
                r"(?:让|要求|通知|提醒).{1,40}(?:提交|提供|反馈|补充).{0,30}(?:说明|材料|资料|文件|信息|表格|报表)",
                text,
            )
        )
