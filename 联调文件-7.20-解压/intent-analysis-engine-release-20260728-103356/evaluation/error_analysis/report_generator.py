    ...
    *,
    ...
    rollback_required = (
        ...
        and (
            ...
            or comparison["negation_false_positive_rate_delta"] > 0
        )
    )
    return {
...
def compare_metrics(before: dict[str, Any], after: dict[str, Any]) -> dict[str, Any]:
    ...
    before_false_positive = float(before.get("false_positive_rate", before_forbidden_rate))
    after_false_positive = float(after.get("false_positive_rate", after_forbidden_rate))
    before_context_recovery = _context_recovery_metric(before)
    after_context_recovery = _context_recovery_metric(after)
    ...
    before_future_fp = float(before.get("future_scope_false_positive_rate", 1 - before_future))
    after_future_fp = float(after.get("future_scope_false_positive_rate", 1 - after_future))
    before_negation_fp = float(before.get("negation_false_positive_rate", 1 - before_negation))
    after_negation_fp = float(after.get("negation_false_positive_rate", 1 - after_negation))
    ...
    return {
        ...
        "context_recovery_accuracy": {
            "before": before_context_recovery,
            "after": after_context_recovery,
        },
        "context_recovery_accuracy_delta": after_context_recovery - before_context_recovery,
        "forbidden_task_rate": {
        ...
        "false_positive_rate": {
            "before": before_false_positive,
            "after": after_false_positive,
        },
        "false_positive_rate_delta": after_false_positive - before_false_positive,
        "future_scope_false_positive": {
            "before": before_future_fp,
            "after": after_future_fp,
        },
        "future_scope_false_positive_delta": after_future_fp - before_future_fp,
        "future_scope_false_positive_rate": {
            "before": before_future_fp,
            "after": after_future_fp,
        },
        "future_scope_false_positive_rate_delta": after_future_fp - before_future_fp,
        "negation_accuracy": {
        ...
        "negation_false_positive_rate": {
            "before": before_negation_fp,
            "after": after_negation_fp,
        },
        "negation_false_positive_rate_delta": after_negation_fp - before_negation_fp,
        ...
        **_metric_delta(before, after, "clarification_decision_accuracy"),
        **_metric_delta(before, after, "clarification_field_accuracy"),
        **_metric_delta(before, after, "clarification_question_accuracy"),
        **_metric_delta(before, after, "no_unnecessary_clarification_question_accuracy"),
        **_metric_delta(before, after, "clarification_recovery_accuracy"),
        **_metric_delta(before, after, "missing_input_precision"),
        **_metric_delta(before, after, "missing_input_recall"),
        **_metric_delta(before, after, "over_clarification_rate"),
    }
...
def _context_recovery_metric(report: dict[str, Any]) -> float:
    if "context_recovery_accuracy" in report:
        return float(report.get("context_recovery_accuracy", 0))
    category = report.get("by_intent_category", {}).get("context_dependency", {})
    total = int(category.get("total", 0) or 0)
    if not total:
        return 0.0
    return float(category.get("passed", 0) or 0) / total
...
def _metric_delta(before: dict[str, Any], after: dict[str, Any], metric: str) -> dict[str, Any]:
    ...
    return {
        ...
        f"{metric}_delta": after_value - before_value,
    }
