...
For a portable API deployment on another computer, use Docker Desktop:

```powershell
.\scripts\start-release.ps1
```

The script creates a local `.env.release` file from the template, starts
PostgreSQL and the API, and exposes:
...
The portable default uses L1 rules plus mock L3 and keeps semantic matching
off, so it does not download vector models. To enable real L3 understanding,
the deployer configures their own API key in `.env.release`.

Create the delivery zip:

```powershell
.\scripts\create-release-package.ps1
```

Full deployment and remote-call instructions:

```text
docs/delivery/RELEASE_RUNBOOK.md
```
