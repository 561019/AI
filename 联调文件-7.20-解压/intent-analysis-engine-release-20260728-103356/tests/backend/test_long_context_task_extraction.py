...
def test_full_layer_outputs_four_standard_tasks_for_reference_sample() -> None:
    ...
    assert analysis.result.tasks[3].missing_inputs == ["calculation_policy"]
...
def test_long_narrative_connectors_extract_requested_actions() -> None:
    result = LongContextTaskExtractionLayer(activation_length=1).extract(
        "先从ERP中获取订单、回款和客户投诉明细，"
        "再按区域汇总订单数量和回款金额。"
        "随后分析华南区域投诉上升原因，比较本季度与上季度收入变化，"
        "同时预测下季度利润。"
        "最后基于分析结果生成经营分析报告。"
    )
...
def test_instructional_context_clauses_do_not_become_tasks() -> None:
    result = LongContextTaskExtractionLayer(activation_length=1).extract(
        "请查询销售数据。"
        "需要保留原始数据口径，汇总时说明去重方式，"
        "分析过程中标记异常字段，记录字段定义和来源。"
        "请分析销售下降原因。"
    )
...
def test_previous_period_comparison_normalizes_to_mom_analysis() -> None:
    result = LongContextTaskExtractionLayer(activation_length=1).extract(
        "请获取经营数据，和上季度比较收入变化，最后生成经营复盘报告。"
    )

    assert any(
        candidate.normalized_text.startswith("环比分析")
        for candidate in result.merged_candidates
    )


def test_long_spoken_business_actions_extract_task_chain() -> None:
    text = (
        "本次只输出任务清单，不执行系统操作。请先把各渠道线索、成交和回款数据拿出来；"
        "再做经营判断，看看哪些渠道转化质量变差；"
        "把异常渠道单独圈出来，按风险高低排；最后形成管理层材料。"
        "不用提醒，也不要进入OA。"
    )

    extraction = LongContextTaskExtractionLayer(activation_length=1).extract(text)
    assert [candidate.action for candidate in extraction.merged_candidates] == [
        "query",
        "analyze",
        "filter",
        "sort",
    ...
    analysis = make_analyzer().analyze_with_debug(
        text=text,
        user_id="user-001",
        conversation_id="long-spoken-actions",
    )
...
def test_long_refund_amount_descending_sort_is_extracted() -> None:
    result = LongContextTaskExtractionLayer(activation_length=1).extract(
        "请先获取本年度门店退款明细，再按城市汇总退款金额，"
        "筛选退款金额异常的门店，按退款金额倒序排序，最后生成退款复盘报告。"
    )
...
def test_long_tasklist_meta_query_uses_chain_context_without_clarification() -> None:
    analysis = make_analyzer().analyze_with_debug(
        text=(
            "请把本年度门店退款数据整理成任务清单：先获取明细，"
            "再按城市汇总退款金额，筛选退款异常门店，"
            "按退款金额倒序排序，最后生成门店退款复盘报告。"
        ),
        user_id="user-001",
        conversation_id="long-tasklist-meta-query",
    )
