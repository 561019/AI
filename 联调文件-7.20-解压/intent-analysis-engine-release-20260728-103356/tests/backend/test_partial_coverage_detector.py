...
def test_detector_splits_english_multi_action_connectors() -> None:
    detector = PartialCoverageDetector()

    segments = detector.segment("retrieve CRM accounts, rank risk, and draft remediation plan")

    assert [segment.text for segment in segments] == [
        "retrieve CRM accounts",
        "rank risk",
        "draft remediation plan",
    ]


def test_detector_ignores_embedded_notification_submission_instruction() -> None:
    detector = PartialCoverageDetector()

    segments = detector.segment("写一封通知，让各区域周五前提交预算说明")

    assert [segment.text for segment in segments] == ["写一封通知"]
