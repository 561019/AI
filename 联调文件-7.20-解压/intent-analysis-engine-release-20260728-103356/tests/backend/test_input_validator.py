...
def test_input_validator_builds_clarification_questions_once() -> None:
    ...
    assert validation.missing_inputs == ["statistical_range"]
    assert validation.clarification_questions == [
        "请确认统计范围（例如时间范围、组织范围）。",
    ]
    ...
    assert validation.missing_input_details[0].source == "task_type_schema"
    validated = validator.validate_task(task)
...
def test_input_validator_distinguishes_uncertain_and_conflicting_inputs() -> None:
    ...
    assert validation.missing_inputs == []
    assert validation.uncertain_inputs == ["calculation_policy"]
    assert validation.conflict_inputs == []
    assert validated.tasks[0].missing_inputs == ["calculation_policy"]
