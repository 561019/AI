...
def test_short_sales_commission_phrase_requires_clarification() -> None:
    ...
    assert analysis.debug["input_validator"]["rules"][0]["source"] == "task_type_schema"
...
def test_single_complaint_plan_generation_does_not_expand_into_workflow() -> None:
    analyzer = make_analyzer()
    ...
    assert [task.task_type for task in result.tasks] == ["IMPROVEMENT_PLAN_GENERATE"]
    assert result.clarification_required is False


def test_complaint_counting_uses_summary_not_information_organize() -> None:
    analyzer = make_analyzer()
    ...
    assert [task.task_type for task in result.tasks] == ["DATA_AGGREGATION_SUMMARY"]
    assert result.tasks[0].missing_inputs == []
    assert result.clarification_required is False


def test_query_object_can_satisfy_data_source_without_extra_clarification() -> None:
    analyzer = make_analyzer()
...
def test_short_generic_requests_are_taskless_clarifications() -> None:
    analyzer = make_analyzer()

    for text in ["按方案来", "处理这些材料", "看看有没有问题"]:
        result = analyzer.analyze(
            text=text,
            user_id="user-001",
...
def test_future_planning_without_current_scope_does_not_create_task() -> None:
    analyzer = make_analyzer()

    result = analyzer.analyze(
        text="未来希望支持异常监控和自动提醒。",
        user_id="user-001",
        conversation_id="conversation-001",
    )
...
def test_later_negation_cancels_previous_candidate_on_rule_path() -> None:
    analyzer = make_analyzer()

    result = analyzer.analyze(
        text="主动提醒负责人。本次任务范围里面不包含异常监控和主动提醒功能。",
        user_id="user-001",
        conversation_id="conversation-001",
    )
...
def test_future_scope_monitoring_exclusion_keeps_current_analysis_task() -> None:
    analyzer = make_analyzer()

    result = analyzer.analyze(
        text="未来规划里会做销售波动监控，目前不包含这部分，当前只需要分析销售波动原因。",
        user_id="user-001",
        conversation_id="conversation-001",
    )

    assert [task.task_type for task in result.tasks] == ["DATA_ANALYSIS_PROBLEM"]
    assert "MONITORING_REMINDER" not in [task.task_type for task in result.tasks]
    assert result.clarification_required is False


def test_negated_report_generation_keeps_positive_analysis_task() -> None:
    analyzer = make_analyzer()
...
def test_negated_reminder_does_not_override_positive_commission_task() -> None:
    analyzer = make_analyzer()

    result = analyzer.analyze(
        text="本次不需要提醒功能，请处理销售提成",
        user_id="user-001",
        conversation_id="conversation-001",
    )

    assert [task.task_type for task in result.tasks] == ["RULE_CALCULATION_COMMISSION"]
    assert "MONITORING_REMINDER" not in [task.task_type for task in result.tasks]


def test_negated_workflow_does_not_create_workflow_task() -> None:
    analyzer = make_analyzer()

    result = analyzer.analyze(
        text="先不要发起流程，帮我看一下销售提成",
        user_id="user-001",
        conversation_id="conversation-001",
    )

    assert [task.task_type for task in result.tasks] == ["RULE_CALCULATION_COMMISSION"]
    assert "WORKFLOW_START" not in [task.task_type for task in result.tasks]
