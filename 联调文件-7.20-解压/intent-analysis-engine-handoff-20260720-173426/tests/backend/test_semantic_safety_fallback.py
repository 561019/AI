...
def test_level3_rejects_unregistered_task_type() -> None:
    text = "把这个特殊事项办妥"
    task = task_payload(
    ...
    reasons = analysis.debug["level3_result"]["validation"]["rejection_reasons"]
    assert "unregistered_task_type:HOTEL_BOOKING" in reasons
    assert analysis.debug["final_decision"]["selected_by"] == "fallback"
