...
class OperationRuleMatcher:
    ...
    def match(self, text: str | dict[str, Any]) -> OperationRuleMatch | None:
        ...
        if self.future_scope_filter.text_is_fully_excluded(normalized):
            return None
        normalized = self.future_scope_filter.remove_excluded_future_scope(normalized).strip()
        if not normalized:
            return None
        ...
        if self._contains_any(normalized, ["排序", "排名"]):
            candidates.append(
                self._single_task_result(
                    text=normalized,
                    rule_name="sort",
    ...
    def _match_document_table_parsing(self, text: str) -> OperationRuleMatch | None:
        ...
        parse_keywords = ["解析", "识别", "读取", "提取", "抽取", "拆解", "导入"]
        structure_keywords = ["结构", "字段", "目录", "条款"]
        field_structure_request = self._contains_any(text, parse_keywords) and self._contains_any(
            text,
            structure_keywords,
        )
        document_parse_request = self._contains_any(text.lower(), document_keywords) and self._contains_any(
            text,
            parse_keywords,
        )
    ...
    def _match_multimedia_generation(self, text: str) -> OperationRuleMatch | None:
        ...
        action_keywords = ["生成", "创建", "制作", "设计", "输出", "画", "做"]
    ...
    def _match_complaint_information_organize(self, text: str) -> OperationRuleMatch | None:
        ...
        organize_keywords = ["整理", "归集", "归类", "归档", "汇总", "统计"]
    ...
    def _match_data_query(self, text: str) -> OperationRuleMatch | None:
        query_keywords = ["查询", "获取", "查看", "拉取", "列出", "导出", "找出", "整理"]
        data_keywords = ["数据", "明细", "记录", "列表", "客户", "订单", "销售", "库存", "金额", "报表"]
    ...
    def _match_knowledge_qa(self, text: str) -> OperationRuleMatch | None:
        knowledge_keywords = ["政策", "制度", "规则", "定义", "说明", "知识库", "faq", "问答", "报销标准", "销售政策"]
    ...
    def _match_content_output(self, text: str) -> OperationRuleMatch | None:
        content_keywords = ["报告", "报表", "文档", "通知", "说明", "方案", "总结", "文案", "邮件", "话术", "材料", "计划"]
        action_keywords = ["生成", "创建", "写", "撰写", "起草", "输出", "制作", "整理成"]
        if not (self._contains_any(text, content_keywords) and self._contains_any(text, action_keywords)):
            return None
    ...
    def _document_inputs(self, text: str) -> list[str]:
        inputs = []
        for keyword in ["Excel", "excel", "PDF", "pdf", "Word", "word", "表格", "合同", "发票", "附件", "文件"]:
            if keyword in text:
    ...
    def _external_system_inputs(self, text: str) -> list[str]:
        ...
        if self._contains_any(text, ["获取", "查询", "拉取"]):
    ...
    def _analytics_inputs(self, text: str, *, method: str) -> list[str]:
        ...
        for keyword in ["销售", "客户投诉", "库存", "利润", "收入", "成本", "风险", "订单"]:
            if keyword in text:
                inputs.append(f"analysis_object:{keyword}")
                break
        return inputs
    ...
    def _content_inputs(self, text: str, *, task_type: str) -> list[str]:
        ...
        for keyword in ["销售", "经营", "客户投诉", "会议", "通知", "报销", "产品"]:
            if keyword in text:
                inputs.append(f"topic:{keyword}")
                break
        return inputs
    ...
    def _has_classification_field(self, text: str) -> bool:
        return self._contains_any(text, ["各区域", "按区域", "区域", "产品", "客户", "部门", "渠道", "分类"])
    ...
    def _has_summary_field(self, text: str) -> bool:
        return self._contains_any(text, ["金额", "数量", "利润", "销售额", "提成", "收入", "成本"])
    ...
    def _has_document_source(self, text: str) -> bool:
        return self._contains_any(text.lower(), ["excel", "pdf", "word", "表格", "文档", "附件", "合同", "发票", "单据", "文件", "上传"])
