...
class DiagnosticTarget(FrozenModel):
    """What phenomenon the diagnostic aims to explain."""
    metric: str = Field(min_length=1, description="e.g. repurchase_rate")
    change: str = Field(min_length=1, description="e.g. -2.0")
    unit: str = Field(default="percentage_points", description="e.g. percentage_points")
    description: str = Field(min_length=1, description="natural-language summary of the issue")


class DiagnosticEntity(FrozenModel):
    """A single entity's metric changes, pre-computed by rule engine."""
    entity_id: str = Field(min_length=1, description="e.g. D001")
    entity_name: str = Field(min_length=1, description="e.g. 经销商A")
    metrics: Mapping[str, str] = Field(
        default_factory=dict,
        description="pre-computed metric values and changes, all as Decimal strings",
    )
    contribution: str | None = Field(
        default=None, description="contribution to overall change, as Decimal string"
    )


class EvidenceRecord(FrozenModel):
    """Evidence record — work reports, service events, etc."""
    evidence_id: str = Field(min_length=1)
    entity_id: str = Field(min_length=1)
    evidence_type: str = Field(min_length=1, description="e.g. delivery_delay, follow_up_missed, work_report")
    summary: str = Field(min_length=1, description="evidence description")
    source_ref: str = Field(min_length=1, description="traceable source reference")
    date: str | None = Field(default=None, pattern=r"^\d{4}-\d{2}-\d{2}$")


class DiagnosticRequest(TraceableModel):
    analysis_type: Literal[AnalysisType.DIAGNOSTIC]
    target: DiagnosticTarget
    entities: tuple[DiagnosticEntity, ...] = Field(min_length=1)
    evidence: tuple[EvidenceRecord, ...] = Field(default_factory=tuple)
    top_n: int = Field(default=2, ge=1, le=20, description="how many major contributors to identify")


AnalysisRequest = Annotated[
    FinancialStatementRequest | PriceForecastRequest | BusinessMetricRequest | DiagnosticRequest,
