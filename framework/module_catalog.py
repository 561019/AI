...
BUSINESS_MODULES: tuple[ModuleSpec, ...] = (
    ModuleSpec(
        ...
        "规则计算引擎",
        8010,
        "/api/v1/rules/instructions",
        "联调文件-7.20-解压/rule_calculation",
        "adapter_registered",
        "L2 规则计算引擎，负责规则核算、规则校验、异常识别和风险输出。",
        ("rule.calculate",),
    ),
    ModuleSpec(
        "content-adapter",
        ...
        "内容产出引擎",
        8011,
        "/api/v1/content/instructions",
        "联调文件-7.20-解压/content_production",
        "adapter_registered",
        "L2 内容产出引擎，负责把各模块业务结果整理成用户可读回答或文件草案。",
        ("content.generate",),
    ),
    ModuleSpec(
    ...
    ModuleSpec(
        ...
        "数据操作引擎",
        8031,
        "/api/v1/data-operation/instructions",
        "联调文件-7.20-解压/2.4-2.12模块和联调--陈宗贤/模块和联调--陈宗贤/数据操作引擎_联调包_v0_2_20260719",
        "adapter_registered",
        "L2 业务数据操作引擎；负责业务数据整合、增删改查、聚合和追踪，物理存取下沉到 L1 数据模块。",
        ("data.collect", "data.consolidate", "data.search", "data.persist", "data.trace", "data.catalog", "data.read", "data.create", "data.update", "data.delete", "data.aggregate"),
    ),
    ModuleSpec(
        "analysis-prediction",
        "business_engine",
        ...
        "联调文件-7.20-解压/analysis-prediction-engine-release-20260729-new",
        "delivered_integrated",
        "已接入最新交付引擎；平台入口为 POST /api/v1/analysis/instructions，内部适配到交付契约。",
